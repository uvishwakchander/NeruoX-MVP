...
function startFocusGame() {
  ...
  renderFocusStats();
  spawnFocusTarget();
  focusEndTimer = setTimeout(() => endFocusGame('Round finished'), 30000);
}
...
function finishMemoryGame(reason) {
  ...
  addXP(Math.max(6, memoryLevel * 3));
...
function endTherapyGame(reason = 'Session complete') {
  ...
  showGameEnd('#therapyEnd', '#therapyEndText', `${reason}. You collected ${calmPoints} calm points.`);
  renderDashboard();
}
